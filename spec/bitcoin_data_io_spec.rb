...
RSpec.describe BitcoinDataIO do
  ...
  describe '#gets_le' do
    it "reads given number of bytes and returns them in little-endian ordering" do
      expect(bytes_to_hex(io('aabbcc').gets_le(2))).to eq 'bbaa'
    end
  end

  describe '#get_le_short' do
    it "reads the next 2 bytes as a little-endian number" do
      expect(io('0201ff').get_le_short).to eq 0x102
    end
  end

  describe '#get_le_integer' do
    it "reads the next 4 bytes as a little-endian number" do
      expect(io('04030201ff').get_le_integer).to eq 0x1020304
    end
  end

  describe '#get_le_long' do
    it "reads the next 8 bytes as a little-endian number" do
      expect(io('0807060504030201ff').get_le_long).to eq 0x102030405060708
    end
  end

  describe '#get_varint' do
    it "properly reads the next 1 byte varint" do
      expect(io('08ff').get_varint).to eq 0x8
    end

    it "properly reads the next 2 byte varint" do
      expect(io('fd0201ff').get_varint).to eq 0x102
    end

    it "properly reads the next 4 byte varint" do
      expect(io('fe04030201ff').get_varint).to eq 0x1020304
    end

    it "properly reads the next 8 byte varint" do
      expect(io('ff0807060504030201ff').get_varint).to eq 0x102030405060708
    end
  end
