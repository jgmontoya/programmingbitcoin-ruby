...
RSpec.describe MerkleHelper do
  ...
  describe '#merkle_parent_level' do
    ...
    context 'when number of hashes is odd' do
      it 'returns the merkle parent level' do
        ...
        bytes_hashes_solution = hex_hashes_solution.map { |h| from_hex_to_bytes(h) }
  ...
  describe '#merkle_root' do
    ...
    it 'computes the merkle root' do
      hex_root_solution = 'acbcab8bcc1af95d8d563b77d24c3d19b18f1486383d75a5085c4e86c86beed6'
      bytes_root_solution = from_hex_to_bytes(hex_root_solution)

      expect(described_class.merkle_root(byte_hashes)).to eq(bytes_root_solution)
    end
  end
end
